<?php
	...
	if($stmt->num_rows == 1){
		...
		$stmt = $mysqli->prepare("SELECT udm_kcal, udm_carb, udm_prot, udm_gord, udc_kcal, udc_carb, udc_prot, udc_gord FROM  v_userdia WHERE usuario_id = ? AND user_dia_data = ?");
		$stmt->bind_param("ss", $user_id, $date);
		$stmt->execute();
		//adicionando todas as colunas em váriáveis
		$stmt->bind_result($udm_kcal, $udm_carb,  $udm_prot, $udm_gord, $udc_kcal, $udc_carb, $udc_prot, $udc_gord);
	...
	//if the day doesn't exist, create it and select it!
		...
		if($stmt->execute()){
			...
			if($stmt->execute()){
				...
				$response['error'] = true;
				$response['message'] = 'erão';
				$stmt->close();
			}
		}else{
			$response['error'] = true;
			$response['message'] = 'erro aqui';
			$stmt->close();
		}
	}else{
		$response['error'] = true;
