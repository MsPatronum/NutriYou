...
/*for ($i = 0; $i < count($keys); $i++){
	...
	if($stmt->num_rows == 1){
		...
		if($stmt->execute()){
			//adding the mood response
			$response['error'] = false;
			$response['message'] = 'Mood inserido com sucesso';
		}
	}else{
		$response['error'] = true;
