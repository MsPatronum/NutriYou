...
Future<ListPatientsModel> fetchPatients() async {
  ...
  var profissionalId = prefs.getInt('id');

  var url = link("profissional/fetch_patients.php");

  var data = {'profissional_id': profissionalId};

  var response = await  http.post(Uri.parse(url), body: json.encode(data)); print(response.body);
  print(data);
  if(response.statusCode == 200){
    
    var stepListModel = listPatientsModelFromJson(response.body);
...
Future removePatient(token) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: buildTitle("Seus pacientes"),
      ...
      body: SafeArea(
        ...
        SingleChildScrollView(
          ...
          child: FutureBuilder<ListPatientsModel>(
                ...
                if(nulo != 0){
                  ...
                  return Column(
                    children: [
                      Container(
                        child: Text(
                          "Ainda não existe nenhum paciente aqui. Adicione!", 
